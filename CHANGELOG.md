...
- Cause: 0.6.3 gated reserved-choice softening on `violates_constraint < ask`, but Jev draws a middling `violates_constraint` on genuine "ask me before" phrasings, so the gate refused to soften a legitimate reservation. It also added no protection: a real prohibition scores `violates_constraint` at deny level and is already `UNSOFTENABLE`, so it denies regardless.
- Fix: the guard now requires only task context and `off_task` below ask (and still never softens `off_task`, and never softens `UNSOFTENABLE` axes). `off_task` is what actually stops a reservation from covering unrelated bundled destruction (set 5 case 16, off_task 0.86 → deny). Rerun pending; set 5 remains development data.
