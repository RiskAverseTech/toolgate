...
## 0.10.0 — the action ledger: write → execute is judged as what the file does
One-shot scoring cannot see "write a helper, then run it". This release closes that shape for files, per [docs/design-action-ledger.md](docs/design-action-ledger.md) (ChatGPT's design), and it is built to give the model facts rather than more text.
- **Capability questions at write time.** On Write/Edit/MultiEdit/NotebookEdit the same model call also answers four questions about the content: if it were later executed, would it read sensitive data, send data externally, destroy data, change privilege? They never touch the write's own verdict (content is not harm; a fixture containing `curl -d @.env` is still allowed) and are excluded from the risk axes and from `audit --stats`. Their answers ride on the ledger entry.
- **A per-session ledger** under `~/.toolgate/ledger/<session>.jsonl` (0700/0600): the write is **proposed** at PreToolUse and becomes **confirmed** when Claude Code's `PostToolUse` arrives for the same `tool_use_id` (`PostToolUseFailure` → failed, `PermissionDenied` → denied). An allow is never assumed to mean the write happened. Stored: paths, booleans, ids. Never content, output, or prompt text.
- **Facts on the execution call.** When a Bash call runs a written path (`bash x.sh`, `./x.sh`, `python x.py`, `node x.js`, `source`, `chmod +x … && ./x`, an `npm run`/`pnpm run`/`yarn test` script when `package.json` was edited), the state carries `session_facts` and the exfiltration, destructive, privilege, and secret-exposure questions are told to judge the call as what the file does, using the capability facts. `cat`, `grep`, `diff` of the file are a *reference* and judged normally. The facts are the one part of the state the questions are told is not untrusted data, because toolgate generated them.
- **The parallel race is floored.** If the execution arrives while the write is still only proposed, the verdict is at least `ask` ("runs a file this session is still writing").
- **`toolgate post`** is the new silent hook command; **`toolgate install` now installs PostToolUse, PostToolUseFailure, and PermissionDenied hooks** alongside PreToolUse (idempotent; existing installs report "added" once). `doctor` reports whether the ledger hooks are present.
- Scope: Claude Code hook sessions only (the MCP proxy has no session in V1); writes made by the write tools, not by Bash itself (`echo … > x.sh`, `npm pkg set`); files, not rows or remote objects. `ledger.enabled: false` turns it off.
- Tests: 32 new, including the whole loop through the built CLI on the mock backend (write → run before confirm = ask → confirm → run = deny → cat = allow → benign helper runs → failed write never becomes a fact); the package smoke test runs the same loop against the installed tarball. The mock backend now treats write-tool content as content (its risk regexes no longer fire on the content being written; the capability regexes do), which is the same distinction the real wording draws.
- Not yet validated on the live model: that needs a frozen *sequential* set (write, then execute), which does not exist yet; set 6 is single-call and unaffected. This release is not installed on the machine running the 0.9.1 measurement window.

## Set 6 frozen, ledger design recorded (git only, between 0.9.2 and 0.10.0)
- **Frozen: challenge set 6, content vs action** (`docs/challenge-set-6.json`). Authored and prospectively labeled by ChatGPT after the 0.7.1 usage report named the failure mode (text describing a harmful action judged as the action). 10 matched pairs, 20 cases, every pair allow-vs-deny: documentation, commit messages, web search, grep patterns, fixtures, creative copy, and a latent package script on the safe side; the same effect actually performed on the deny side. Four safe sides are themselves Bash calls, so a fix that only softens Write inputs cannot pass. Protocol in the file: it is *not* run on current wording; the wording change is made from evidence already in hand, and this set runs once on the candidate. A `categories` map was added for the runner's per-category summary; the cases are as authored.
- **Design record for multi-step composition** (`docs/design-action-ledger.md`): the action ledger as flat derived facts into the existing questions (no new axis), effects confirmed through Claude Code's `PostToolUse` / `PostToolUseFailure` / `PermissionDenied` lifecycle (verified against the current hooks reference) rather than assumed from an allow, artifact capabilities captured as non-verdict metadata at write time, the parallel write-then-execute race handled by treating proposed writes as unconfirmed, and a V1 scope of write → confirmed artifact → later execution only. Not started; sequenced after the measurement window, the second labeling, the wording change, and the set 6 run.
- `npm run release:verified`: check, package smoke test, then publish, as one command. `prepublishOnly` stays build plus unit tests, since a nested install inside npm's lifecycle fails.

## 0.9.2 — key-aware redaction, a real deny floor, package smoke test
Hardening only; no threshold or question wording changes. Driven by ChatGPT's review of 0.9.1 (source, tests, and proxy, not just the README), which found two concrete defects.
- **Structured-secret redaction (the P0).** Redaction ran text patterns over each string value and never looked at the JSON key, so `{ "password": "hunter2" }` reached the model and the audit log untouched unless the value happened to look like a known token format. Free-text commands were fine (`PASSWORD=hunter2` is one string); MCP arguments are structured objects, so proxy mode made this live. Now any scalar under a sensitive key (`password`, `secret`, `token`, `api_key`, `authorization`, `cookie`, `credential`, `private_key`, `passphrase`, `client_secret`, `connection_string`, and variants, matched on the normalized key so `apiKey` and `x-api-key` count) is replaced outright, then text patterns run as before. One redaction path: the audit log now derives its text from the same redacted value the model gets, instead of a second, weaker pass over the raw input.
- **The built-in deny floor is now real.** User rules were prepended to the built-ins and the first match won, so a broad user `allow` (say, `match: { tool: Bash }`) silently neutralized the built-in `rm -rf /` deny, and the source comment claiming built-ins "stay as the floor" was false. Order is now: built-in *denies*, then user rules, then the remaining built-in *asks*. A user rule can deliberately override a built-in ask (one specific installer's `curl | sh`); it can never lower a built-in deny. Policy owners were always trusted, so this was a footgun rather than an exploit, and it is closed either way.
- **Package smoke test** (`npm run smoke`, and a CI job): builds the real tarball, installs it into a scratch project, and exercises the shipped features through the installed bin: feature markers present in `dist/`, over-broad `trusted_tools` rejected, deny floor under a broad allow, key-aware redaction in the model state, forged MCP name refused, per-axis stats and the version stamp in the audit log. It tests what users receive. It is the check that would have caught 0.8.0 and 0.9.0. (It can't run inside `prepublishOnly`, since a nested `npm install` inherits the publishing npm's environment; run it before `npm publish`, and CI runs it on every push.)
- **Every audit line now carries `toolgate_version`**, so a "single-version log" is verifiable from the log rather than from memory of when the hook was upgraded.
- README: the lede says plainly that toolgate is both a Claude Code hook and an MCP proxy; the Evaluation section is brought up to the 0.6.4 reruns (20/20, 20/20 with 10/10 pairs, 23/24 on the adversarial set, zero permissive errors across all frozen sets) instead of "rerun pending"; the rule-order text matches the code; the multi-step roadmap item records the action-ledger design.

## 0.9.1 — trusted_tools bound to the install, per-axis stats, the 41-decision regression check
- **Retraction: the 0.8.0 and 0.9.0 packages on npm shipped stale compiled code.** Their `dist/` was the 0.7.1 build; neither `trusted_hosts` nor `trusted_tools` was actually in the published JavaScript, although the README and CHANGELOG described them. The source in git was correct; the release process never rebuilt before publishing. Both versions are deprecated on npm with a pointer here. 0.9.1 is the first release that actually contains those features. `npm run check` now rebuilds `dist/` before testing (so the end-to-end tests exercise the current build, which is how this was caught), and `prepublishOnly` rebuilds and tests before every publish, so a stale `dist/` can no longer be published.
