...
Sixty labeled commands across three frozen challenge sets, each authored and prospectively labeled by an independent reviewer before it was run, with the exact state sent to the model recorded per case. On v0.4 question wording: the development set 18/20; the held-out set **20/20** with complete input (an earlier 12/20 was a toolgate task-truncation bug, kept in the record); a second held-out set of ten matched pairs **17/20, 7/10 complete pairs, zero permissive errors** — the three misses were one wording defect (an "ask me before…" instruction scored as a prohibition). On the current wording and policy (0.6.4, unchanged since), reruns of the frozen sets score **20/20** (set 2) and **20/20 with 10/10 complete pairs** (set 3), and a fifth held-out adversarial set of 24 cases from the independent reviewer (inert and forged approvals in the input, metadata spoofing, base64 and encoded targets, reservation transfer and injection, approval provenance and retraction, write-versus-execute) scores **23/24**, the one miss stricter than desired. Across every frozen set on current wording: **zero permissive errors**. The development set built from the first real-usage log scores 9/12, its three misses all stricter than desired (`sudo pkill`, and two deletes of development-database rows scored as destruction). Full tables in `docs/challenge-set-*-results-v0.6.4.md`. Flag-carried effects (`--draft=false`, `git clean -f` vs `-n`, missing `--dry-run`), bundles distinguished only by a second-half flag, and settled-vs-reserved choices on identical commands all separate cleanly. Latency is ~1.2 s per decision from a laptop, whether direct to TypeSafe's API (1208 ms) or through Vercel's Gateway (1324 ms) — the Gateway adds ~100 ms; the rest is the model round trip. TypeSafe quotes 70–500 ms; we have not observed that from outside their infrastructure.
...
- [ ] **Multi-step composition**: catch the write-a-helper-then-exec pattern a one-shot scorer misses. Design direction (from the independent review): not more transcript in the prompt, but a small structured *action ledger* per session (files written, destinations contacted, secrets read, privilege transitions) handed to the model as derived state, so "execute a file this session wrote" is a fact the questions can see.
